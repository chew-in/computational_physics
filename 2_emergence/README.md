...
where $\beta=(k_\beta T)^{-1}$, and $Z_\beta$ is the partition function. For a function $f$ of spins ("observable"), one can calculate the expectation (mean) value of $f$ denoted by $\gt f \lt < > _\beta$ using the the configuration probability, as follows,
$$\<f\>_\beta$$
